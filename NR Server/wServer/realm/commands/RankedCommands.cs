...
namespace wServer.realm.commands
{
    ...
    class GiveClass : Command
    {
        public GiveClass() : base("class", permLevel: 90, alias: "cls") { }

        protected override bool Process(Player player, RealmTime time, string args)
        {
           string input = args.ToLower();
           ...
           switch(input)
           {
                case "Knight":
                    if (player != null)
                    {
                        GiveItem(player, 0xa6c);
                        GiveItem(player, 0xa0c);
                        GiveItem(player, 0xa12);
                        GiveItem(player, 0xac7);
        ...
        protected void GiveItem(Player player,ushort objtype)
        {
            for (var i = 4; i< player.Inventory.Length; i++)
            {
                if (player.Inventory[i] == null)
                {
                    player.Inventory[i] = player.Manager.Resources.GameData.Items[objtype];
                    return;
                }
            }
        }
    ...
    {
        public SellCommand() : base("Sell", permLevel: 90, alias: "sll") { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (slotnumber == null || slot < 4 || player.Inventory[slot].FeedPower == 0)
            {
                player.SendError("Cannot Sell: Emty Inv, Price : 0, Eqt slot");
                return false;
            }
            else
            {
                player.Credits = acc.Credits += player.Inventory[slot].FeedPower;
                player.SendInfo(player.Inventory[slot].FeedPower.ToString());
    ...
    class SetGoldCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            player.Credits = acc.Credits += amount2;
    ...
    {
        public SetFameCommand() : base("setfame", permLevel: 90, alias: "fame") { }
        ...
        protected override bool Process(Player player, RealmTime time, string args)
        {
            ...
            if (string.IsNullOrEmpty(args))
            {
                player.SendHelp("/setfame <amount>");
                return false;
            }
            player.Fame = account.Fame += amount;
            player.ForceUpdate(account.Fame);
