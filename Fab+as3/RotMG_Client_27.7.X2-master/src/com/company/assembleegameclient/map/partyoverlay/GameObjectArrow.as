package com.company.assembleegameclient.map.partyoverlay
{
   ...
   public class GameObjectArrow extends Sprite
   {
      ...
      public function draw(param1:int, param2:Camera) : void
      {
         ...
         _local3 = this.correctQuestNote(param2.clipRect_);
         ...
         else if(this.tempPoint.x > _local3.right - 5)
         {
            if(_local6 > 0)
            {
               if(_local6 < 135)
         ...
         else if(this.tempPoint.y > _local3.bottom - 5)
         {
            if(_local6 > -45)
            {
               _local6 = -45;
      ...
      {
         var _local2:Rectangle = param1.clone();
         if(stage.scaleMode == StageScaleMode.NO_SCALE)
         {
            if(Boolean(Parameters.data_["uiscale"]) || false)
            {
            ...
            _local2.right = _local2.right - (800 - this.go_.map_.gs_.hudView.x) * stage.stageWidth / Parameters.data_["mscale"] / 800;
         }
         else
         {
            this.scaleX = this.scaleY = 1;
      ...
      private function positionTooltip(param1:ToolTip) : void
      {
         ...
         var _local6:Number = param1.contentWidth_;
         var _local7:Number = param1.contentHeight_;
         if(_local2 >= 45 && _local2 <= 135)
         {
            _local8 = _local4 + _local6 / Math.tan(_local2 * Trig.toRadians);
            param1.x = (_local4 + _local8) / 2 - _local6 / 2;
            param1.y = _local5;
         }
         else if(_local2 <= -45 && _local2 >= -135)
         {
            _local8 = _local4 - _local6 / Math.tan(_local2 * Trig.toRadians);
            param1.x = (_local4 + _local8) / 2 - _local6 / 2;
            param1.y = _local5 - _local7;
         }
         else if(_local2 < 45 && _local2 > -45)
         {
            param1.x = _local4;
            _local9 = _local5 + _local7 * Math.tan(_local2 * Trig.toRadians);
            param1.y = (_local5 + _local9) / 2 - _local7 / 2;
         }
         else
         {
            param1.x = _local4 - _local6;
            _local9 = _local5 - _local7 * Math.tan(_local2 * Trig.toRadians);
            param1.y = (_local5 + _local9) / 2 - _local7 / 2;
         }
      }
      ...
      private function drawArrow() : void
      {
         ...
         _local1.clear();
         var _local2:int = Boolean(this.big_) || Boolean(this.mouseOver_)?int(int(BIG_SIZE)):int(int(SMALL_SIZE));
