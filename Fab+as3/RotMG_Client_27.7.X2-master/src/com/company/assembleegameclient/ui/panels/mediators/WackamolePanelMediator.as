...
import kabam.rotmg.messaging.impl.outgoing.WackaMole.EnterWackamole;
...
public class WackamolePanelMediator extends Mediator {

    public static const TEXT:String = "SellableObjectPanelMediator.text";

    [Inject]
    public var view:WackamolePanel;
    [Inject]
    ...
    private function onPurchase(_arg_1:int):void {
        if ((_arg_1 == Currency.GOLD)) {
            this.purchaseWithCoin();
        }
    }


    private function purchaseWithCoin():void {
        var _local_2:EnterWackamole;
        if (!this.account.isRegistered()) {
            ...
            if (!this.gameModel.player.nameChosen_) {
                ...
                if (this.gameModel.player.credits_ < 50) {
                    ...
                    _local_2 = (this.messages.require(GameServerConnection.ENTER_WACKAMOLE) as EnterWackamole);
