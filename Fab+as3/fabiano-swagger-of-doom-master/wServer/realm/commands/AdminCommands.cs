...
namespace wServer.realm.commands
{
            ...
            {
                {
                    ...
                    {
                        player.SendError("Please use this command in the realm portal");
                        return false;
                    }
                }
                {
    ...
    internal class SprintCommand : Command
    {
        public SprintCommand() : base("sprint", 0) { }

        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (player.CanSprint == false)
            {               
                player.SendError("You do not have the required stamina.");
                return false;
            }
            else
            {
                player.SendInfo("Spring On!");
                player.Sprint = true;
            }
            return true;
        }
    ...
    {
        public TpQuestCommand() : base("tpq", 1) { }
    ...
    internal class PetYardCommand :Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            player.Manager.Database.DoActionAsync(db =>
            {
                ...
                cmd.CommandText = "UPDATE accounts SET petYardType=5 WHERE id=@accId;";
    ...
    internal class ArenaCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            Entity prtal = Entity.Resolve(player.Manager, "Abyss of Demons Portal"); //change this l8r m8
            ...
            if (!player.Manager.GameData.Items[objType].Secret || player.Client.Account.Rank >= 4)
            {
                for (int i = 4; i < player.Inventory.Length; i++)
                    if (player.Inventory[i] == null)
                    {
                        ...
                        player.SendInfo("You have successfully given yourself " + name);
                        break;
                    }
            }
            else
    ...
    internal class SWhoCommand : Command
    {
        public SWhoCommand()
            : base("swho", 1)
        {
            ...
            foreach (KeyValuePair<string, Client> i in player.Manager.Clients)
            {
                ...
                if (i.Value.Player.Name.EqualsIgnoreCase(args[0]))
                {
                    ...
                    else
                    {
                        ...
                        player.SendInfo( Name + " will connect to you now!");
                    }
    ...
    internal class TqCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            ...
            player.Move(player.Quest.X + 1f, player.Quest.Y + 1f);
            player.ApplyConditionEffect(new ConditionEffect()
            {
                Effect = ConditionEffectIndex.Invulnerable,
                DurationMS = 200,
    ...
    internal class StarCommand : Command
    {
        ...
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            try
            {
                ...
                {
                    player.Client.Player.Stars =  star;
    ...
    internal class SetSCommand : Command
    {
        public SetSCommand()
            : base("setStat", 1)
        {
