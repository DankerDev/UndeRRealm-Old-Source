...
namespace wServer.realm
{
    internal class Oryx : IDisposable
    {
        ...
        {
            if (CountEnemies("Red Demon") != 0) return false;
            RealmClosed = true;
            return true;
        }
        ...
        public void Init()
        {
            ...
            foreach (var i in spawn)
            {
                ...
                var enemyCount = stats[idx] / i.Value.Item1 / 2;
        ...
        {
            player.SendInfo("You've entered Under Realms");
            player.SendEnemy("Demon the King of UnderRealms", "Hello Their");
            player.SendEnemy("Oryx", "Master... let me handle this.");
        ...
        {
            log.Info("Demon is controlling population...");
            ...
            GC.Collect();
            log.Info("Demon is back to sleep.");
        }
        ...
        private void SpawnEvent(string name, ISetPiece setpiece)
        {
            ...
            log.InfoFormat("Demon spawned {0} at ({1}, {2}).", name, pt.X, pt.Y);
        }
