    ...
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            src.Owner.BroadcastPacketSync(new TextPacket()
            {
                Name = 
                (
                src.Client.Account.Rank >= 6 ? "@[Owner]": src.Client.Account.Rank >= 5 ? "@[Dev]" : src.Client.Account.Rank >= 4 ? "@[Admin]" : 
                src.Client.Account.Rank >= 3 ? "#[SD]" : src.Client.Account.Rank >= 2 ? "#[D]" : src.Client.Account.Rank >= 1 ? "#[YT]" : ""
                ) + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
