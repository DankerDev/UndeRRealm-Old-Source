...
namespace wServer.realm.entities.player
{
    public partial class Player
    {
        ...
        {
            if (level == 1) return 0;
            return 60 * (level - 1) + (level - 2) * (level - 1) * 60;
        }


        private static int GetFameGoal(int fame)
        {
            if (fame >= 6000) return 0;
            if (fame >= 2000) return 6000;
        ...
        private Entity FindQuest()
        {
            ...
            try
            {
                    ...
                    .OrderBy(quest => MathsUtils.DistSqr(quest.X, quest.Y, X, Y)).Where(i => i.ObjectDesc != null && i.ObjectDesc.Quest))
                {
                    ...
                    var score = (20 - Math.Abs((i.ObjectDesc.Level ?? 0) - Level))*x.Item1 - Dist(this, i)/100; 
                    if (score < 0)
                        score = 1;
        ...
        {
            int newFame;
            if (Experience < 20*100) newFame = Experience/2500;
            else newFame = 20 + (Experience - 20*100)/2500;
            ...
            if (newGoal > FameGoal)
            {
                ...
                {
                    ObjectId = Id,
                    Color = new ARGB(0x84FAF6),
                    Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"Fame Goal Complete!\"}}",
                }, null);
                Stars = GetStars();
            }
            FameGoal = newGoal;
        ...
        {
            if (Experience - GetLevelExp(Level) >= ExperienceGoal && Level < 200)
            {
                Level++;
                ...
                foreach (var i in Manager.GameData.ObjectTypeToElement[ObjectType].Elements("LevelIncrease"))
                {
                    ...
                    var limit =
                        int.Parse(xElement.Attribute("max").Value);
                    var idx = StatsManager.StatsNameToIndex(i.Value);
                    if (Level > 20) Stats[idx] += 0;
                    else Stats[idx] += rand.Next(min);
                ...
                if (Level == 200)
                {
                    foreach (var i in Owner.Players.Values)
                        i.SendInfo(Name + " has achieved level 200");
                    XpBoosted = false;
                    XpBoostTimeLeft = 0;
                }
                CalculateFame();
        ...
        public bool EnemyKilled(Enemy enemy, int exp, bool killer)
        {
            if (enemy == Quest)
                ...
                {
                    ObjectId = Id,
                    Color = new ARGB(0x7F0000),
                    Text = "{\"key\":\"blank\",\"tokens\":{\"data\":\"Boss Slain!\"}}",
                }, null);
            if (exp > 0)
            {
                if(XpBoosted)
