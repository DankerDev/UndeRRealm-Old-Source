...
namespace wServer.realm.entities.player
{
    partial class Player
    {
        ...
        {
            double arcGap = item.ArcGap * Math.PI / 180 + 0.5f;
            ...
            if (item.ArcGap * Math.PI / 180 + 0.5f != arcGap)
            {
                Client.Disconnect();
            }
        }
        ...
        public bool Activate(RealmTime time, Item item, UseItemPacket pkt)
        {
            ...
            Mp -= item.MpCost;
            HP -= item.HpCost;
            if (Mp - item.MpCost < 0 || HP - item.HpCost <10 || HasConditionEffect(ConditionEffects.Quiet) || HasConditionEffect(ConditionEffects.Paused))
            {
                return true;
            }
            else
            {
                ...
                foreach (ActivateEffect eff in item.ActivateEffects)
                {
                    switch (eff.Effect)
                    {
                        ...
                        case ActivateEffects.ReaperAbility:
                            {
                                if (!reaperinvis)
                                {
                                    ApplyConditionEffect(new ConditionEffect
