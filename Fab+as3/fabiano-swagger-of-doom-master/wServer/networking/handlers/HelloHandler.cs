...
namespace wServer.networking.handlers
{
    internal class HelloHandler : PacketHandlerBase<HelloPacket>
    {
        ...
        protected override void HandlePacket(Client client, HelloPacket packet)
        {
            ...
            {
                client.Player.SendError("Please use the correct client version");
                client.Disconnect();
                return;
            }
            ...
            if (client.Account.Name.Length > 10)
            {
                client.Disconnect();
                    client.SendPacket(new FailurePacket
                    {
                        ErrorId = 6969,
                        ErrorDescription = "Your name is too long for the servers comfort."     
                    });
                }
                if (!client.Account.NameChosen)
                {
                    ...
                    {
                        ErrorId = 0,
                        ErrorDescription = "Create a name in character select screen!"
                ...
                else
                {
                    ...
                    if (world == null && packet.GameId == World.ENTRANCE_ID) world = client.Manager.AddWorld(new WackaMoleEntrance(false));
                    if (world == null)
                    {
                        client.SendPacket(new FailurePacket
                        {
